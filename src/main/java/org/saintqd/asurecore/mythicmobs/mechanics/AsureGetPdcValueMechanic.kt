...
@MythicMechanic(author = "SaintQd", name = "asuregetpdcvalue")
class AsureGetPdcValueMechanic(event : MythicMechanicLoadEvent) : ITargetedEntitySkill {

    val key = NamespacedKey.fromString(event.config.getPlaceholderString(arrayOf("name", "n"), "").get())!!

    val name : String = event.config.getString(arrayOf("name", "n"),"asure_${key.key}")
    ...
    override fun castAtEntity(
        ...
        AsureUtils.sendDebugMessage(3,"MythicMobsMechanic: asuregetpdcvalue")
