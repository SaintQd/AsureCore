...
import org.saintqd.asurecore.managers.ItemSkinManager;
...
public class ItemSkinGUI extends AsureGUI {
    ...
    public void setItemSkinsMenu(ItemStack itemStack, int page) {
        ...
        if (itemStack.getType() == Material.AIR) {
            getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "item_skin_gui_item_hint"));
            return;
        }

        int size = 45;
        int loopIndex = 0;
        setInventory(Bukkit.createInventory(new AsureGUIHolder(this), size,
                AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "item_skin_gui_title")));
        getButtons().clear();
        ...
        for (String skinName : availableSkins.keySet()) {
            ...
            if (possibleTypeFound) {
                ...
                skinItem.setData(DataComponentTypes.CUSTOM_NAME, AsureUtils.parseString(itemSkin.displayName()));
                skinItem.setData(DataComponentTypes.ITEM_MODEL, itemSkin.model());

                String[] loreLines = LangManager.INSTANCE.getRawLangString(AsureCore.inst(),"item_skin_gui_item_lore").split("<newline>");
                for (String line : loreLines)
                    lore.addLine(AsureUtils.parseString(line));

                skinItem.setData(DataComponentTypes.LORE,lore.build());

                AsureGUIButton showButton = new AsureGUIButton().consumer(event -> {
        ...
        if (slotIndex==0) {
            ...
            ItemLore.Builder lore = ItemLore.lore();
            emptyItem.setData(DataComponentTypes.CUSTOM_NAME,AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "item_skin_gui_empty"));
            String[] loreLines = LangManager.INSTANCE.getRawLangString(AsureCore.inst(),"item_skin_gui_empty_lore").split("<newline>");
            for (String line : loreLines)
                lore.addLine(AsureUtils.parseString(line));
        ...
        ItemLore.Builder lore = ItemLore.lore();
        defaultItem.setData(DataComponentTypes.CUSTOM_NAME,AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "item_skin_gui_reset"));
        String[] loreLines = LangManager.INSTANCE.getRawLangString(AsureCore.inst(),"item_skin_gui_reset_lore").split("<newline>");
        for (String line : loreLines)
            lore.addLine(AsureUtils.parseString(line));
        defaultItem.setData(DataComponentTypes.LORE,lore.build());

        AsureGUIButton removeButton = new AsureGUIButton().consumer(event -> {
            removeItemSkin();
    ...
    private void setItemSkin(String skinName) {
        ...
        if (!possibleTypeFound) {
            getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "item_skin_wrong_type"));
            return;
        }
        ...
        getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "item_skin_success"));
    }
    ...
    private void removeItemSkin() {
        ...
        if (handItem.getPersistentDataContainer().has(ItemSkinManager.ITEM_SKIN_KEY)) {
            ...
            getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "item_skin_reset"));
        }
        else {
            getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "item_skin_reset_not_set"));
        }
    }
}
