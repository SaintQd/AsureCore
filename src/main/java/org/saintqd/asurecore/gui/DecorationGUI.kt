...
import org.saintqd.asurecore.managers.DecorationManager
...
class DecorationGUI(player: Player) : AsureGUI(player) {
    ...
    fun setDecorationMenu(itemStack: ItemStack?) {
        ...
        inventory = Bukkit.createInventory(
            ...
            AsureLib.inst().langManager.parseLangString(AsureCore.inst(), "item_skin_gui_title"))
        buttons.clear()
        ...
        if (player.hasPermission("asurecore.admin") || player.isOp)
            ...
            availableDecorations.forEach { (decorationName, state) ->
                ...
                decorationElement.materials.forEach { (material, neededAmount) ->
                    ...
                    if (currentAmount >= neededAmount)
                        lore.addLine(AsureUtils.parseString("<green>✓ $currentAmount/$neededAmount <reset><white>$translationString"));
                    else {
                        craftCheck = false
                        lore.addLine(AsureUtils.parseString("<red>✗ $currentAmount/$neededAmount <reset><white>$translationString"));
                    }
                }

                if (craftCheck) {
                    lore.addLine { Component.empty() }
                    val loreLines = LangManager.INSTANCE.getRawLangString(AsureCore.inst(),"decoration_gui_item_lore")
                        .split("<newline>")
        ...
        if (slotIndex==0) {
            ...
            emptyItem.setData(DataComponentTypes.CUSTOM_NAME,AsureLib.inst().langManager.parseLangString(AsureCore.inst(), "decoration_gui_empty"));

            val lore = ItemLore.lore()
            val loreLines = LangManager.INSTANCE.getRawLangString(AsureCore.inst(),"decoration_gui_empty_lore")
                .split("<newline>")
            loreLines.forEach { line ->
                lore.addLine(AsureUtils.parseString(line))
            }
        ...
        if (selectItem.type == Material.YELLOW_STAINED_GLASS_PANE)
            selectItem.setData(DataComponentTypes.CUSTOM_NAME,
                AsureLib.inst().langManager.parseLangString(AsureCore.inst(), "decoration_gui_insert"))

        val lore = ItemLore.lore()
        val loreLines = LangManager.INSTANCE.getRawLangString(AsureCore.inst(),"decoration_gui_insert_lore")
            .split("<newline>")
        loreLines.forEach { line ->
            lore.addLine(AsureUtils.parseString(line))
        }
        ...
        buttons[size - 5] = AsureGUIButton().consumer { event ->
    ...
    private fun createDecorationItem(decorationName : String) {
        ...
        if (matsMap.isNotEmpty()) {
            player.sendMessage(AsureLib.inst().langManager.parseLangString(AsureCore.inst(), "decoration_no_items"))
            return
        }
