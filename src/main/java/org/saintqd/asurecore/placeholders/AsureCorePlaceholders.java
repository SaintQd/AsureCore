package org.saintqd.asurecore.placeholders;
...
public class AsureCorePlaceholders extends PlaceholderExpansion {

    private final AsureCore plugin;
    private final HashMap<String, BiFunction<AsureCore,OfflinePlayer,String>> placeholders = new HashMap<>();
    ...
    public AsureCorePlaceholders(AsureCore plugin){
        this.plugin = plugin;
    }
    ...
    private HashMap<String,String> getOfflinePlayerPlaceholders(OfflinePlayer offlinePlayer) {
        if (offlinePlayerPlaceholders.containsKey(offlinePlayer.getName())) {
            ...
            if (user != null) {
                ...
                if (AsureCore.inst().getLuckPermsManager() != null) {
                    CompletableFuture<String> completableFuture = AsureCore.inst().getLuckPermsManager().getOfflineVipUntil(player.getUniqueId());
    ...
    public void registerPlaceholders() {
        ...
        placeholders.put("vip_until",(plugin,player) -> {
            if (player.isOnline()) {
                if (AsureLib.inst().getVaultManager() != null && AsureLib.inst().getVaultManager().getPermissionProvider() != null
                        && AsureCore.inst().getLuckPermsManager() != null) {
                    return AsureCore.inst().getLuckPermsManager().getVipUntil(player.getPlayer());
                }
                else
                    return "";
            }
        ...
        placeholders.put("suffix",(plugin,player) -> {
            if (AsureCore.inst().getLuckPermsManager() != null)
                return AsureCore.inst().getLuckPermsManager().getSuffix(player.getPlayer());
            else return "";
        });
        ...
        placeholders.put("suffix_symbol_from_luckperms",(plugin,player) -> {
            String suffixName = AsureCore.inst().getSuffixManager().getSuffixPlaceholdersToNames().getOrDefault(
        ...
        placeholders.put("confirmation_status",(plugin,player) -> {
            ...
            if (player.getPlayer().permissionValue(permission) == TriState.TRUE) {
                return PlaceholderAPI.setPlaceholders(player,
                        PlaceholderAPI.setPlaceholders(player, AsureCore.inst().getConfig().getString("ConfirmationStatus.Format","")));
            }
            else
                return "";
        });
