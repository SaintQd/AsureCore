...
import org.saintqd.asurecore.gui.ArmorStandGUI;
...
public class PlayerListener implements Listener {
    ...
    private final NamespacedKey SAVED_HEALTH_KEY = new NamespacedKey(AsureCore.inst(), "saved_health");
    ...
    public void onPlayerLogin(AsyncPlayerPreLoginEvent event) {
        if (!AsureCore.inst().getConfig().getBoolean("PlayerLimit.Enabled"))
            return;
        ...
        if (AsureLib.inst().getVaultManager() != null && AsureLib.inst().getVaultManager().getPermissionProvider() != null) {
            if (AsureLib.inst().getVaultManager().getPermissionProvider().playerHas(null,offlinePlayer,"asurecore.bypasslimit")) {
                return;
            }
        }
        if (Bukkit.getOnlinePlayers().size() + 1 > AsureCore.inst().getConfig().getInt("PlayerLimit.Limit",100)) {
            event.kickMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"kick_server_is_full"));
            event.setLoginResult(AsyncPlayerPreLoginEvent.Result.KICK_FULL);
        }
        if (event.getLoginResult() == AsyncPlayerPreLoginEvent.Result.ALLOWED
                && AsureCore.inst().getConfig().getBoolean("CommandsAfterLastSeen.Enabled")
                && AsureCore.inst().getConfig().contains("CommandsAfterLastSeen.Presets")) {
            long lastSeen = offlinePlayer.getLastSeen();
            if (lastSeen > 0) {
                long checkDelay = AsureCore.inst().getConfig().getLong("CommandsAfterLastSeen.CheckDelay", 60L);
                Bukkit.getScheduler().runTaskLater(AsureCore.inst(), () -> {
                    if (!offlinePlayer.isOnline() || offlinePlayer.getName() == null)
                        return;
                    for (String presetName : AsureCore.inst().getConfig().getConfigurationSection("CommandsAfterLastSeen.Presets").getKeys(false)) {
                        ConfigurationSection presetConfig = AsureCore.inst().getConfig().getConfigurationSection("CommandsAfterLastSeen.Presets." + presetName);
    ...
    public void onPlayerJoinAccepted(final PlayerJoinEvent event) {
        if (AsureCore.inst().getConfigManager().getAccountTransferNewToOldNicknames().containsKey(event.getPlayer().getName())) {
            String oldNickname = AsureCore.inst().getConfigManager().getAccountTransferNewToOldNicknames().get(event.getPlayer().getName());
            Bukkit.getScheduler().runTaskLater(AsureCore.inst(),() -> {
                event.getPlayer().kick(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"account_transfer_kick_second_message"));
                Bukkit.getScheduler().runTask(AsureCore.inst(),() -> {
                    AsureCore.inst().getConfigManager().performAccountTransfer(null,oldNickname,event.getPlayer().getName());
                    String oldPlayerName = AsureCore.inst().getConfigManager().getAccountTransferNewToOldNicknames().remove(event.getPlayer().getName());
                    AsureCore.inst().getConfigManager().getPendingAccountTransfers().remove(oldPlayerName);
                });
            },20L);
        }
    }
    ...
    public void onPlayerJoin(final PlayerJoinEvent event) {
        AsureCorePlaceholders.getOfflinePlayerPlaceholders().remove(event.getPlayer().getName());
        AsureCore.inst().getSuffixManager().checkSuffixPermission(event.getPlayer());

        if (AsureCore.inst().getConfig().getBoolean("Tweaks.HealthSave",true)) {
            Bukkit.getScheduler().runTaskLater(AsureCore.inst(), () -> {
        ...
        int hintIndex = AsureCore.inst().getConfig().getInt("StarterHints.Join",-2);
        if (hintIndex > -2 && event.getPlayer().permissionValue("asurecore.disablehints") != TriState.TRUE && AsureCore.inst().isCMIEnabled()) {
            long maxPlaytime = AsureCore.inst().getConfig().getLong("StarterHints.MaxPlaytime",18000000);
            ...
            if (user.getTotalPlayTime() > maxPlaytime)
                playTimeCheck = false;
            if (playTimeCheck) {
                if (hintIndex == -1 || hintIndex >= AsureCore.inst().getHintManager().getHints().size())
                    AsureCore.inst().getHintManager().sendStarterHint(Audience.audience(event.getPlayer()));
                else
                    AsureCore.inst().getHintManager().sendStarterHint(Audience.audience(event.getPlayer()), hintIndex);
            }
        }
        ...
        if (mailbookManager.getUnreadMailbooks().containsKey(event.getPlayer().getUniqueId())) {
            event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"mailbook_receive_message"));
        }
        ...
        if (CalendarEventsManager.Companion.getInstance().getCalendar().containsKey(LocalDate.now().getDayOfYear())) {
            ...
            event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"calendar_event_message_format"));
            for (String eventName : currentEventsList) {
                CalendarEventsManager.AsureCalendarEvent timedEvent = CalendarEventsManager.Companion.getInstance().getEvents().get(eventName);
                event.getPlayer().sendMessage(AsureLib.inst().getLangManager().
                        parseLangString(AsureCore.inst(),"calendar_event_message_list_format",timedEvent.getDisplayName()));
            }
            event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"calendar_event_message_hint_format"));
        }

        if (AsureCore.inst().getConfig().getBoolean("Guide.Enabled",true) && AsureCore.inst().getConfig().getBoolean("Guide.ShowMessage",true)) {
            int ticksPlayed = event.getPlayer().getStatistic(Statistic.PLAY_ONE_MINUTE);
            if (ticksPlayed < AsureCore.inst().getConfig().getInt("Guide.MaxTimePlayedToShowMessage",72000)) {
                event.getPlayer().sendMessage(LangManager.INSTANCE.parseLangString(AsureCore.inst(),"guide_possible_message"));
            }
        }

        if (event.getPlayer().permissionValue("asurecore.pvpenabled") == TriState.TRUE) {
            PlayerManager playerManager = AsureCore.inst().getPlayerManager();
            playerManager.getPvpModePlayers().add(event.getPlayer());
        }

        if (AsureCore.inst().getConfig().getBoolean("Messages.Enabled")) {
            event.joinMessage(null);
            String joinMessage = null;
            String joinMessageFormat = null;
            if (event.getPlayer().hasPlayedBefore()) {
                if (event.getPlayer().permissionValue("asurecore.hidejoinmessage") == TriState.TRUE)
                    return;
                // Фикс скрытия сообщений входа/выхода для системы ваниша в CMI
                if (AsureCore.inst().isCMIEnabled()) {
                ...
                if (!possibleJoinMessage.isEmpty()
                        && event.getPlayer().hasPermission("asurecore.joinmessage")
                        && AsureCore.inst().getConfig().getBoolean("Messages.Join.Enabled")) {
                    ...
                    joinMessageFormat = AsureCore.inst().getConfig().getString("Messages.Join.Format", "<white>>> <gray>[message]");
                    for (String groupName : AsureCore.inst().getConfig().getConfigurationSection("Messages.Join.FormatPerGroup").getKeys(false)) {
                        if (event.getPlayer().hasPermission("group."+groupName))
                            joinMessageFormat = AsureCore.inst().getConfig().getString("Messages.Join.FormatPerGroup."+groupName, "<white>>> <gray>[message]");
                    }
                } else if (AsureCore.inst().getConfig().getBoolean("Messages.DefaultJoin.Enabled")) {
                    joinMessage = AsureCore.inst().getConfig().getString("Messages.DefaultJoin.Format", null);
                    for (String groupName : AsureCore.inst().getConfig().getConfigurationSection("Messages.DefaultJoin.FormatPerGroup").getKeys(false)) {
                        if (event.getPlayer().hasPermission("group."+groupName)) {
                            joinMessage = AsureCore.inst().getConfig().getString("Messages.DefaultJoin.FormatPerGroup."+groupName, null);
                        }
                    }
                    joinMessageFormat = joinMessage;
                }
            } else if (AsureCore.inst().getConfig().getBoolean("Messages.FirstJoin.Enabled")) {
                joinMessage = AsureCore.inst().getConfig().getString("Messages.FirstJoin.Format", null);
                joinMessageFormat = joinMessage;
            }
            if (joinMessage == null || joinMessage.isEmpty())
                return;
            joinMessageFormat = joinMessageFormat.replace("[message]", joinMessage).replace("[dot]", ".");
            joinMessageFormat = joinMessageFormat.replace("*", AsureCore.inst().getConfig().getString("Messages.NicknameFormat", event.getPlayer().getName()));
            ...
            joinMessageFormat = (AsureCore.inst().getPlaceholders() != null)
                    ...
                    : joinMessageFormat;
            event.joinMessage(AsureUtils.parseString(joinMessageFormat));
        }
    }
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        if (playerManager.getPvpModePlayers().contains(event.getPlayer())) {
            vaultManager.getPermissionProvider().playerAdd(null,event.getPlayer(),"asurecore.pvpenabled");
            playerManager.getPvpModePlayers().remove(event.getPlayer());
        }
        else {
            vaultManager.getPermissionProvider().playerRemove(null,event.getPlayer(),"asurecore.pvpenabled");
        }

        if (AsureCore.inst().getConfig().getBoolean("Messages.Enabled")) {
            event.quitMessage(null);
            if (event.getPlayer().permissionValue("asurecore.hideleavemessage") == TriState.TRUE)
                return;
            // Фикс скрытия сообщений входа/выхода для системы ваниша в CMI
            if (AsureCore.inst().isCMIEnabled()) {
                CMIVanish vanish = CMI.getInstance().getVanishManager().getVanish(event.getPlayer().getUniqueId());
            ...
            if (!possibleLeaveMessage.isEmpty()
                    && event.getPlayer().hasPermission("asurecore.leavemessage")
                    && AsureCore.inst().getConfig().getBoolean("Messages.Leave.Enabled")) {
                ...
                leaveMessageFormat = AsureCore.inst().getConfig().getString("Messages.Leave.Format", "<white<<< <gray>[message]");
                for (String groupName : AsureCore.inst().getConfig().getConfigurationSection("Messages.Leave.FormatPerGroup").getKeys(false)) {
                    if (event.getPlayer().hasPermission("group."+groupName))
                        leaveMessageFormat = AsureCore.inst().getConfig().getString("Messages.Leave.FormatPerGroup."+groupName, "<white>>> <gray>[message]");
                }
            } else if (AsureCore.inst().getConfig().getBoolean("Messages.DefaultLeave.Enabled")) {
                leaveMessage = AsureCore.inst().getConfig().getString("Messages.DefaultLeave.Format", null);
                for (String groupName : AsureCore.inst().getConfig().getConfigurationSection("Messages.DefaultLeave.FormatPerGroup").getKeys(false)) {
                    if (event.getPlayer().hasPermission("group."+groupName))
                        leaveMessage = AsureCore.inst().getConfig().getString("Messages.DefaultLeave.FormatPerGroup."+groupName, null);
                }
            ...
            leaveMessageFormat = leaveMessageFormat.replace("*", AsureCore.inst().getConfig().getString("Messages.NicknameFormat", event.getPlayer().getName()));
            leaveMessageFormat = (AsureCore.inst().getPlaceholders() != null)
                    ...
                    : leaveMessageFormat;
            event.quitMessage(AsureUtils.parseString(leaveMessageFormat));
        }
    }
    ...
    public void onPlayerInteract(PlayerInteractEntityEvent event) {
        if (event.getRightClicked() instanceof Player interactedPlayer) {
            if (!AsureCore.inst().getConfig().getBoolean("Messages.RightClickNickname.Enabled")) return;
            if (AsureCore.inst().getConfig().getBoolean("Messages.RightClickNickname.SneakingAllowed")
                    && event.getPlayer().isSneaking()) return;
            if (!(interactedPlayer.hasPotionEffect(PotionEffectType.INVISIBILITY)
            && AsureCore.inst().getConfig().getBoolean("Messages.RightClickNickname.HideInvisible"))
            || AsureCore.inst().getConfig().getStringList("Messages.RightClickNickname.AlwaysShowGamemodes")
                    .contains(event.getPlayer().getGameMode().name())
            || event.getPlayer().hasPermission("asurecore.rightclicknickname.alwaysshow")) {
                String messageFormat = AsureCore.inst().getConfig().getString("Messages.NicknameFormat", event.getPlayer().getName());
                messageFormat = (AsureCore.inst().getPlaceholders() != null)
                        ? PlaceholderAPI.setPlaceholders(interactedPlayer, PlaceholderAPI.setPlaceholders(interactedPlayer,messageFormat))
                        : messageFormat;
                event.getPlayer().sendActionBar(AsureUtils.parseString(messageFormat));
            }
        }
        if (event.getRightClicked() instanceof Breedable breedable) {
            if (!AsureCore.inst().getConfig().getBoolean("Tweaks.AgeLock.Enabled",true)) return;
            if (breedable.getAgeLock() || breedable.isAdult()) return;
            ItemStack handItem = event.getPlayer().getInventory().getItemInMainHand();
            if (handItem.getType() !=
                    Material.valueOf(AsureCore.inst().getConfig().getString("Tweaks.AgeLock.Item",Material.POISONOUS_POTATO.name()))) return;
            if (AsureCore.inst().getConfig().getBoolean("Tweaks.AgeLock.RemoveItem",true) && event.getPlayer().getGameMode() != GameMode.CREATIVE)
    ...
    public void onPlayerBlockInteract(PlayerInteractEvent event) {
        ...
        if (AsureCore.inst().getPlayerManager().getKnockoutPlayers().containsKey(event.getPlayer().getUniqueId())) {
            event.setCancelled(true);
            return;
        }
            ...
            if (event.getClickedBlock() != null) {
                switch (event.getClickedBlock().getType()) {
                    case CHIPPED_ANVIL,DAMAGED_ANVIL -> {
                        if (AsureCore.inst().getConfig().getBoolean("Tweaks.AnvilRepair.Enabled", true)) {
                            Material repairMaterial = Material.valueOf(AsureCore.inst().getConfig().getString("Tweaks.AnvilRepair.RepairMaterial", "IRON_BLOCK"));
                            @Subst("block.chain.hit") String repairSound = AsureCore.inst().getConfig()
                                    .getString("Tweaks.AnvilRepair.RepairSound", null);
                            if (!event.getPlayer().hasPermission("asurecore.anvilrepair")) return;
                    ...
                    case PODZOL -> {
                        if (AsureCore.inst().getConfig().getBoolean("Tweaks.RemovePodzol.Enabled", true)) {
                            ItemStack mainHandItem = event.getPlayer().getInventory().getItemInMainHand();
                            Optional<Material> possibleRequiredHandMaterial = Enums.getIfPresent(Material.class,
                                    AsureCore.inst().getConfig().getString("Tweaks.RemovePodzol.HandMaterial", "BRUSH"));
                            if (possibleRequiredHandMaterial.isPresent()) {
                                ...
                                if (mainHandItem.getType() == requiredHandMaterial) {
                                    int replaceRadius = AsureCore.inst().getConfig().getInt("Tweaks.RemovePodzol.Radius", 1);
                                    ...
                                    if (AsureCore.inst().getConfig().getBoolean("Tweaks.RemovePodzol.SpawnParticles", true)) {
                ...
                if (logsTag != null) {
                    if (logsTag.isTagged(event.getClickedBlock().getType())) {
                        if (AsureCore.inst().getConfig().getBoolean("Tweaks.UnstripLogs.Enabled", true)) {
                            Optional<Material> possibleRequiredHandMaterial = Enums.getIfPresent(Material.class,
                                    AsureCore.inst().getConfig().getString("Tweaks.UnstripLogs.HandMaterial", "BONE_MEAL"));
                            if (possibleRequiredHandMaterial.isPresent()) {
                                Material requiredHandMaterial = possibleRequiredHandMaterial.get();
                                int neededAmount = AsureCore.inst().getConfig().getInt("Tweaks.UnstripLogs.Amount", 1);
                                ItemStack handItem = event.getPlayer().getInventory().getItemInMainHand();
                                if (handItem.getType() == requiredHandMaterial && handItem.getAmount() >= neededAmount) {
                                    ...
                                    if (originalMaterialName.contains("STRIPPED")) {
                                        ...
                                        if (possibleMaterial.isPresent()) {
                                            ...
                                            if (AsureCore.inst().getConfig().getBoolean("Tweaks.UnstripLogs.SpawnParticles", true)) {
                                                ParticleBuilder particleBuilder = new ParticleBuilder(Particle.COMPOSTER);
                                                particleBuilder.count(25);
                                                particleBuilder.offset(0.5,0.5,0.5);
                                            ...
                                            String possibleSoundData = AsureCore.inst().getConfig().getString(
                                                    "Tweaks.UnstripLogs.Sound","item.axe.strip,2");
                                            if (!possibleSoundData.isEmpty()) {
                                                String[] soundData = AsureCore.inst().getConfig().getString(
        ...
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK || event.getAction() == Action.RIGHT_CLICK_AIR) {
            if (AsureCore.inst().getConfig().getBoolean("ArmorStandInteractions.Enabled", true)) {
                ItemStack mainHandItem = event.getPlayer().getInventory().getItemInMainHand();
                String showNearItemMaterialName = AsureCore.inst().getConfig().getString("ArmorStandInteractions.ShowNearItem", Material.ECHO_SHARD.name());
                int radius = AsureCore.inst().getConfig().getInt("ArmorStandInteractions.ShowNearRadius", 10);
                int delay = AsureCore.inst().getConfig().getInt("ArmorStandInteractions.ShowNearGlowTime", 200);
                if (mainHandItem.getType().name().equals(showNearItemMaterialName)) {
                    if (!event.getPlayer().hasPermission("asurecore.armorstandinteractions.shownear") &&
                            !event.getPlayer().hasPermission("asurecore.admin")) {
                        event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "no_permission"));
                        return;
                    }
    ...
    public void onPlayerDamage(EntityDamageByEntityEvent event) {
        ...
        if (AsureCore.inst().getPlayerManager().getKnockoutPlayers().containsKey(event.getEntity().getUniqueId())) {
            event.setCancelled(true);
        }
        if (AsureCore.inst().getPlayerManager().getKnockoutPlayers().containsKey(damagerEntity.getUniqueId())) {
            event.setCancelled(true);
        }
        ...
        if (event.getEntity() instanceof Player entityPlayer && damagerEntity instanceof Player damagerPlayer) {
            ...
            if (playerManager.isPvpModeEnabled()) {
                if (damagerPlayer.hasPermission("asurecore.admin"))
                    return;
                if (!playerManager.getPvpModePlayers().contains(entityPlayer)) {
                    ...
                    if (damagerVariable.getRight() < AsureUtils.getCurrentTick()) {
                        damagerVariable = new ImmutablePair<>(null, AsureUtils.getCurrentTick() +
                                AsureCore.inst().getConfig().getLong("TimersCooldown.entity_not_enabled_pvp", 200L));
                        timers.put(damagerPlayer, damagerVariable);
                        playerManager.getTimers().put("entity_not_enabled_pvp", timers);
                        damagerPlayer.sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "entity_not_enabled_pvp", entityPlayer.getName()));
                    }
                    return;
                }
                if (!playerManager.getPvpModePlayers().contains(damagerPlayer)) {
                    ...
                    if (damagerVariable.getRight() < AsureUtils.getCurrentTick()) {
                        damagerVariable = new ImmutablePair<>(null, AsureUtils.getCurrentTick() +
                                AsureCore.inst().getConfig().getLong("TimersCooldown.damager_not_enabled_pvp", 200L));
                        timers.put(damagerPlayer, damagerVariable);
                        playerManager.getTimers().put("damager_not_enabled_pvp", timers);
                        damagerPlayer.sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "damager_not_enabled_pvp", entityPlayer.getName()));
                    }
                    return;
                }
            }
            if (AsureCore.inst().getConfig().getBoolean("Tweaks.DeathKnockout.Enabled", true)) {
                if (event.getFinalDamage() >= entityPlayer.getHealth() && entityPlayer.permissionValue("asurecore.deathknockout") != TriState.TRUE) {
                    if (!AsureCore.inst().getPlayerManager().getKnockoutPlayers().containsKey(event.getEntity().getUniqueId())) {
                        if (entityPlayer != damagerPlayer) {
                            ...
                            AsureCore.inst().getPlayerManager().applyKnockout(entityPlayer, damagerPlayer);
                        }
                    }
                }
            }
    ...
    public void anvilEvent(PrepareAnvilEvent event) {
        ...
        if (renameText != null && !renameText.isEmpty()
                && AsureCore.inst().getConfig().getBoolean("CommandFilter.Enabled") && !event.getView().getPlayer().hasPermission("asurecore.commandfilter.bypass")) {
            for (String regex : AsureCore.inst().getConfig().getStringList("CommandFilter.Regex")) {
        ...
        if (!AsureCore.inst().getConfig().getBoolean("Tweaks.RepairCostFix.Enabled")) return;
        ...
        event.getView().setRepairCost(
                AsureCore.inst().getConfig().getBoolean("Tweaks.RepairCostFix.Force",false)
                ? 39
    ...
    public void onPlayerGlide(EntityToggleGlideEvent event) {
        if (event.getEntity() instanceof Player player && AsureCore.inst().isWorldGuardEnabled()) {
            ...
            if (!container.createQuery().testState(localPlayer.getLocation(),localPlayer, Flags.GLIDE)) {
                ...
                if (chestplate != null && chestplate.getType() == Material.ELYTRA) {
                    NamespacedKey key = new NamespacedKey(AsureCore.inst(), AsureCore.inst().getConfig()
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        if (AsureCore.inst().getConfig().getBoolean("Tweaks.HeadDrops",true)) {
            if (event.getPlayer().getKiller() != null && event.getPlayer().permissionValue("asurecore.headdrop") == TriState.TRUE) {
                Player killer = event.getPlayer().getKiller();
                ItemStack headItem = ItemStack.of(Material.PLAYER_HEAD);
                headItem.setData(DataComponentTypes.LORE, ItemLore.lore()
                        .addLine(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "head_lore", killer.getName()))
                        .build());
        ...
        if (AsureCore.inst().getConfig().getBoolean("Tweaks.HideDeathMessages.Enabled",true)) {
            long period = AsureCore.inst().getConfig().getLong("Tweaks.HideDeathMessages.Period",12000);
            int minDeaths = AsureCore.inst().getConfig().getInt("Tweaks.HideDeathMessages.MinDeaths",3);
            Set<Long> deathCounterPerPlayer = deathCounter.getOrDefault(event.getEntity().getUniqueId(),new HashSet<>());
            deathCounterPerPlayer.removeIf( time -> time + period < AsureUtils.getCurrentTick());
            deathCounterPerPlayer.add(AsureUtils.getCurrentTick());
        ...
        if (!AsureCore.inst().getConfig().getBoolean("Messages.Death.Enabled",true)) return;
        ...
        String prefix = AsureCore.inst().getConfig().getString("Messages.Death.Format","");

        for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
            if (serializedMessage.contains(">"+onlinePlayer.getName()+"\":")) {
                String playerName = AsureCore.inst().getConfig().getString("Messages.Death.PlayerName","*")
                        .replace("*",onlinePlayer.getName());
                playerName = AsureLib.inst().isPlaceholderAPIEnabled()
        ...
        event.deathMessage(AsureUtils.parseString(serializedMessage));
    }
    ...
    public void onAdvancementReceive(PlayerAdvancementDoneEvent event) {
        ...
        if (!AsureCore.inst().getConfig().getBoolean("Messages.Advancement.Enabled",true)) return;

        List<String> showTypes = AsureCore.inst().getConfig().getStringList("Messages.Advancement.ShowTypes");
        if (!AsureCore.inst().getConfig().getStringList("Messages.Advancement.AlwaysShowNamespaces")
        ...
        String prefix = AsureCore.inst().getConfig().getString("Messages.Advancement.Format","");

        String playerName = AsureCore.inst().getConfig().getString("Messages.Advancement.PlayerName","*")
        ...
        serializedMessage = prefix.replace("*",serializedMessage);
        event.message(AsureUtils.parseString(serializedMessage));
    }

    /*@EventHandler(priority = EventPriority.MONITOR)
    public void onPlayerJoinDynamic(PlayerJoinEvent event) {
        AsureCore.inst().getDynamicParamsManager().updateWorldCaps(Bukkit.getOnlinePlayers().size());
    }

    @EventHandler(priority = EventPriority.MONITOR)
    public void onPlayerQuitDynamic(PlayerQuitEvent event) {
        AsureCore.inst().getDynamicParamsManager().updateWorldCaps(Bukkit.getOnlinePlayers().size() - 1);
    }*/
    ...
    public void onGrindstoneClick(InventoryClickEvent event) {
        ...
        if (!AsureCore.inst().getConfig().getBoolean("Tweaks.GrindstoneDisenchant",true)) return;
        if (!event.getWhoClicked().hasPermission("asurecore.grindstonedisenchant")) return;
    ...
    public void onPlayerBlockBreak(BlockBreakEvent event) {
        if (event.isCancelled())
            return;
        if (AsureCore.inst().getConfig().getBoolean("OreAlerts.Enabled")) {
            if (AsureCore.inst().getOreManager().getThresholdMaterials().containsKey(event.getBlock().getType())) {
                if (event.getPlayer().permissionValue("asurecore.orealerts.bypass") == TriState.TRUE)
                    return;
                if (event.getPlayer().getGameMode() == GameMode.CREATIVE || event.getPlayer().getGameMode() == GameMode.SPECTATOR)
                    return;
                Location blockLoc = event.getBlock().getLocation();
                if (AsureCore.inst().getOreManager().getCheckedLocations().containsKey(blockLoc)
                        || AsureCore.inst().getOreManager().getPlacedOres().contains(blockLoc))
                    return;
                AsureCore.inst().getOreManager().getPlacedOres().remove(blockLoc);
                //String stringLoc = blockLoc.getWorld().getName()+","+blockLoc.x()+","+blockLoc.y()+","+blockLoc.z();
                //if (AsureCore.inst().getOreManager().getCheckedLocations().containsKey(stringLoc))
                //    return;

                long currentTick = AsureUtils.getCurrentTick();
                long timeToThreshold = AsureCore.inst().getConfig().getLong("OreAlerts.TimeToThreshold", 12000);
                ...
                for (Location location : blockLocations)
                    AsureCore.inst().getOreManager().getCheckedLocations().put(location, currentTick);
                List<OreManager.OreData> oreData = AsureCore.inst().getOreManager().getPlayerOreData().getOrDefault(event.getPlayer().getName(), new ArrayList<>());
                ...
                AsureCore.inst().getOreManager().getPlayerOreData().put(event.getPlayer().getName(), oreData);
                List<Player> alertedPlayers = new ArrayList<>(Bukkit.getOnlinePlayers().stream()
                        .filter(player -> player.hasPermission("asurecore.orealerts.show")).toList());
                String parsedOreName = AsureCore.inst().getOreManager()
                        .getThresholdMaterials().get(event.getBlock().getType()).getRight() + "<lang:" + event.getBlock().getType().translationKey() + ">";
                String hoverCommand = AsureCore.inst().getConfig().getString("OreAlerts.HoverCommand", "tp {1}")
                        .replace("{1}", event.getPlayer().getName());
                String hoverText = "<click:run_command:\"/" + hoverCommand + "\"><hover:show_text:'" + AsureLib.inst().getLangManager().getLangLines()
                        .get(Key.key("asurecore:ore_alert_hover_tooltip")) + "'>" + AsureLib.inst().getLangManager().getLangLines().get(Key.key("asurecore:ore_alert_hover")) + "</hover></click>";
                Component smallAlertComponent = AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "ore_alert_small",
                        event.getPlayer().getName(), Integer.toString(blockLocations.size()), parsedOreName, hoverText);
                for (Player player : alertedPlayers) {
                    if (player.permissionValue("asurecore.orealerts.disablesmall") != TriState.TRUE) {
                        player.sendMessage(smallAlertComponent);
                        if (player.permissionValue("asurecore.orealerts.disablesmallsound") != TriState.TRUE) {
                            player.playSound(AsureCore.inst().getOreManager().getAlertSound(), player);
                        }
                    }
                }
                if (currentMaterialAmount >= AsureCore.inst().getOreManager().getThresholdMaterials().get(event.getBlock().getType()).getLeft()) {
                    long timeRange = currentTick - oldestData;
                    // Перевод в минуты. Всегда отображается минимум 1 минута
                    timeRange = timeRange / 20 / 60;
                    if (timeRange <= 0)
                        timeRange = 1;
                    Component largeAlertComponent = AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "ore_alert_large",
                            event.getPlayer().getName(), Integer.toString(currentMaterialAmount), parsedOreName, Long.toString(timeRange), hoverText);
                    for (Player player : alertedPlayers) {
                        if (player.permissionValue("asurecore.orealerts.disablelarge") != TriState.TRUE) {
                            player.sendMessage(largeAlertComponent);
                            if (player.permissionValue("asurecore.orealerts.disablelargesound") != TriState.TRUE) {
                                player.playSound(AsureCore.inst().getOreManager().getAlertSound(), player);
                            }
                        }
                    }
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        if (AsureCore.inst().getConfig().getBoolean("OreAlerts.Enabled")) {
            if (AsureCore.inst().getOreManager().getThresholdMaterials().containsKey(event.getBlock().getType())) {
                Location blockLoc = event.getBlock().getLocation();
                AsureCore.inst().getOreManager().getCheckedLocations().remove(blockLoc);
                AsureCore.inst().getOreManager().getPlacedOres().add(blockLoc);
            }
        }
        if (AsureCore.inst().getConfig().getBoolean("ShulkerDupeDetector.Enabled")) {
            if (event.getBlock().getType().name().contains(Material.SHULKER_BOX.name()) && event.getBlock().getState() instanceof TileStateInventoryHolder state) {
                ...
                if (shulkerItem.getPersistentDataContainer().has(ShulkerAlertManager.SHULKER_UUID_KEY)) {
                    ...
                    state.update();
                    long keepUuidTime = AsureCore.inst().getConfig().getLong("ShulkerDupeDetector.KeepUuidTime",12000L);
                    if (!event.getPlayer().hasPermission("asurecore.shulkerdupealerts.bypass")) {
                        if (ShulkerAlertManager.INSTANCE.getWorldShulkerUuids().containsKey(shulkerUuidString)) {
                            ...
                            if (time + keepUuidTime > AsureUtils.getCurrentTick()) {
                        ...
                        ShulkerAlertManager.INSTANCE.getWorldShulkerUuids().put(shulkerUuidString, AsureUtils.getCurrentTick());
                    }
                }
                else {
    ...
    public void onBlockBreakBlock(BlockBreakBlockEvent event) {
        if (AsureCore.inst().getConfig().getBoolean("ShulkerDupeDetector.Enabled") && !event.getDrops().isEmpty()) {
    ...
    public void onPlayerPortalTeleport(PlayerPortalEvent event) {
        if (AsureCore.inst().isWorldGuardEnabled()) {
            com.sk89q.worldguard.protection.regions.RegionContainer container = com.sk89q.worldguard.WorldGuard.getInstance().getPlatform().getRegionContainer();
        ...
        if (AsureCore.inst().getConfig().getBoolean("Tweaks.PortalRedirect.Enabled")) {
            if (AsureCore.inst().getConfig().contains("Tweaks.PortalRedirect.Worlds." + event.getFrom().getWorld().getName())) {
                event.setCancelled(true);
                for (String command : AsureCore.inst().getConfig().getStringList("Tweaks.PortalRedirect.Worlds." + event.getFrom().getWorld().getName() + ".Commands")) {
                    Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), command.replace("%player_name%", event.getPlayer().getName()));
                }
            }
        }
        if (AsureCore.inst().getConfig().getBoolean("CustomPortals.Enabled") && event.getCause() == PlayerTeleportEvent.TeleportCause.NETHER_PORTAL) {
            ...
            for (Material foundBlockType : foundCornerBlocks.keySet()) {
                if (AsureCore.inst().getConfig().contains("CustomPortals.List."+foundBlockType.name())) {
                    int requiredAmount = AsureCore.inst().getConfig().getInt("CustomPortals.List."+foundBlockType.name()+".Amount",1);
                    if (foundCornerBlocks.get(foundBlockType) >= requiredAmount) {
                        event.setCancelled(true);
                        for (String command : AsureCore.inst().getConfig().getStringList("CustomPortals.List."+foundBlockType.name()+".Commands")) {
                            Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), command.replace("%player_name%", event.getPlayer().getName()));
                        }
                    }
                }
    ...
    public void onEntityPortalTeleport(EntityPortalEvent event) {
        if (event.getEntity() instanceof Player)
            return;
        if (!AsureCore.inst().getConfig().getBoolean("Tweaks.EntityPortalBlock.Enabled"))
            return;
        if ((event.getPortalType() == PortalType.NETHER && AsureCore.inst().getConfig().getBoolean("Tweaks.EntityPortalBlock.EnabledNether"))
            || (event.getPortalType() == PortalType.ENDER && AsureCore.inst().getConfig().getBoolean("Tweaks.EntityPortalBlock.EnabledEnd"))) {
            event.setCancelled(true);
            if (AsureCore.inst().getConfig().getBoolean("Tweaks.EntityPortalBlock.Remove"))
    ...
    public void onPlayerMaceDamage(EntityDamageByEntityEvent event) {
        if (!AsureCore.inst().getConfig().getBoolean("Tweaks.MaceDenier.Enabled"))
            return;
        if (event.getDamager() instanceof Player player && player.getInventory().getItemInMainHand().getType() == Material.MACE) {
            ...
            if (entityCustomNameComponent != null) {
                ...
                if (AsureCore.inst().getConfigManager().getMaceDenierCustomNames().contains(name))
                    event.setDamage(1);
            }
        }
    }
    ...
    public void onPlayerCraft(PrepareItemCraftEvent event) {
        ...
        if (originalResultItem != null) {
            if (AsureCore.inst().getConfigManager().getItemLockMaterials().contains(originalResultItem.getType().name())) {
            ...
            /*
            if (AsureCore.inst().getConfig().getBoolean("ShulkerDupeDetector.Enabled",true)
    ...
    public void onCartographyTableCraft(CartographyItemEvent event) {
        if (event.getInventory().getResult() != null && AsureCore.inst().getConfigManager()
    ...
    public void onPlayerCommandExecute(PlayerCommandPreprocessEvent event) {
        if (AsureCore.inst().getConfig().getBoolean("CommandFilter.Enabled") && !event.getPlayer().hasPermission("asurecore.commandfilter.bypass")) {
            for (String regex : AsureCore.inst().getConfig().getStringList("CommandFilter.Regex")) {
                Pattern pattern = Pattern.compile(regex);
                Matcher matcher = pattern.matcher(event.getMessage());
                if (matcher.find()) {
                    event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "command_filter_message"));
                    event.setCancelled(true);
                    return;
                }
            }
        ...
        if (handItem.getType() != Material.AIR) {
            if (handItem.getPersistentDataContainer().has(ConfigManager.getLockKey())) {
                List<String> blockedCommands = AsureCore.inst().getConfig().getStringList("Tweaks.ItemLock.BlockedCommands");
                for (String command : blockedCommands) {
                    if (event.getMessage().startsWith("/" + command)) {
                        event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "lock_item_command_blocked"));
                        event.setCancelled(true);
                        return;
                    }
                }
            ...
            if (handItem.getPersistentDataContainer().has(ConfigManager.getSignKey())) {
                List<String> blockedCommands = AsureCore.inst().getConfig().getStringList("Tweaks.ItemSign.BlockedCommands");
                for (String command : blockedCommands) {
                    if (event.getMessage().startsWith("/" + command)) {
                        event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "sign_item_command_blocked"));
                        event.setCancelled(true);
                        return;
                    }
                }
        ...
        if (event.getItem().getPersistentDataContainer().has(ItemSkinManager.ITEM_SKIN_KEY)) {
            ...
            if (itemSkin != null) {
                if (!itemSkin.permission().isEmpty() && !event.getPlayer().hasPermission(itemSkin.permission())) {
                    ...
                    event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "item_skin_no_permission"));
                }
            }
        }
    }
    ...
    public void onPlayerItemDrop(PlayerDropItemEvent event) {
        ...
        if (AsureCore.inst().getConfigManager().getCauldronTransforms().containsKey(originalType)) {
            Item droppedItem = event.getItemDrop();
            Material requiredMaterial = AsureCore.inst().getConfig().getBoolean("Tweaks.CauldronTransform.RequireWater", true)
                    ? Material.WATER_CAULDRON
                    : Material.CAULDRON;
            Material newMaterial = Material.valueOf(AsureCore.inst().getConfigManager().getCauldronTransforms().get(originalType));
            Bukkit.getScheduler().scheduleSyncDelayedTask(AsureCore.inst(), () -> {
    ...
    public void onVaultOpen(VaultChangeStateEvent event) {
        if (event.getPlayer() != null) {
            if (AsureCore.inst().getConfig().getBoolean("Tweaks.VaultReset.Enabled",true)) {
                int cooldown = AsureCore.inst().getConfig().getInt("Tweaks.VaultReset.Cooldown",600);
    ...
    public void onInteractWithArmorStand(PlayerInteractAtEntityEvent event) {
        ...
        if (AsureCore.inst().getConfig().getString("ArmorStandInteractions.ChangeNameItem",Material.NAME_TAG.name()).equals(handItemType.name())) {
            event.setCancelled(true);
            if (!event.getPlayer().hasPermission("asurecore.armorstandinteractions.setname") &&
                    !event.getPlayer().hasPermission("asurecore.admin")) {
                event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "no_permission"));
                return;
            }
        ...
        else if (AsureCore.inst().getConfig().getString("ArmorStandInteractions.OpenMenuItem",Material.IRON_INGOT.name()).equals(handItemType.name())) {
            event.setCancelled(true);
