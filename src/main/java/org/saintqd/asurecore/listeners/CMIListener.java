...
public class CMIListener implements Listener {
    ...
    public void onCMIVanish(CMIPlayerVanishEvent event) {
        ...
        if (user.getVanish().getState(VanishAction.fakeJoinLeave).is()) {
            ...
            }.runTaskLater(AsureCore.inst(),1L);
            ...
            Audience.audience(Bukkit.getOnlinePlayers()).sendMessage(AsureUtils.parseString(leaveMessageFormat));
        }
    }
    ...
    public void onCMIUnVanish(CMIPlayerUnVanishEvent event) {
        ...
        if (user.getVanish().getState(VanishAction.fakeJoinLeave).is()) {
            ...
            joinMessageFormat = (AsureCore.inst().getPlaceholders() != null)
                    ...
                    : joinMessageFormat;
            Audience.audience(Bukkit.getOnlinePlayers()).sendMessage(AsureUtils.parseString(joinMessageFormat));
        }
    }
}
