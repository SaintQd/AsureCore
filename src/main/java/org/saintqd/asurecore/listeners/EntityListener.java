...
public class EntityListener implements Listener {
    ...
    private final NamespacedKey COPPER_GOLEM_LIMIT_KEY = new NamespacedKey(AsureCore.inst(),"copper_golem_limit");
    ...
    public void onCopperGolemSpawn(EntitySpawnEvent event) {
        if (event.getEntity() instanceof CopperGolem copperGolem
                ...
                && AsureCore.inst().getConfig().getBoolean("Tweaks.CopperGolem.Enabled",true)) {
            copperGolem.setAware(false);
            List<String> activationMaterials = AsureCore.inst().getConfig().getStringList("Tweaks.CopperGolem.InteractMaterials");
            ...
            audience.sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"copper_golem_interact_hint",translatableKey));
        }
    }
    ...
    public void onCopperGolemInteract(PlayerInteractEntityEvent event) {
        ...
        if (!AsureCore.inst().getConfig().getBoolean("Tweaks.CopperGolem.Enabled",true)) return;

        List<String> possibleInteractMaterials = AsureCore.inst().getConfig().getStringList("Tweaks.CopperGolem.InteractMaterials");
        ...
        if (copperGolem.isAware()) {
            if (possibleInteractMaterials.contains(event.getPlayer().getInventory().getItemInMainHand().getType().name())) {
                if (!event.getPlayer().hasPermission("asurecore.admin")) {
                    ...
                    if (!uuids.contains(golemUuid)) {
                        event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "copper_golem_not_owner"));
                        return;
                    }
                ...
                event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"copper_golem_interact_off"));
            ...
            if (possibleInteractMaterials.contains(event.getPlayer().getInventory().getItemInMainHand().getType().name())) {
                if (!event.getPlayer().hasPermission("asurecore.admin")) {
                    ...
                    if (uuids.size() == AsureCore.inst().getConfig()
                            .getInt("Tweaks.CopperGolem.MaxPerPlayer", 2)) {
                        event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "copper_golem_limit_reached"));
                        return;
                    }
                ...
                if (interactSound != null)
                    copperGolem.getWorld().playSound(copperGolem.getLocation(),interactSound,SoundCategory.NEUTRAL, 1f,1f);
                event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"copper_golem_interact_on"));
    ...
    public void onDisabledBlockDestroy(BlockDestroyEvent event) {
        ...
        if (AsureCore.inst().getConfigManager().getDisabledDrops().contains(blockType)) {
