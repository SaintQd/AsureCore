...
import org.saintqd.asurecore.managers.PlayerManager;
...
public class VillagerListener implements Listener {
    ...
    public void onVillagerInteract(PlayerInteractEntityEvent event) {
        ...
        if (!container.createQuery().testState(localPlayer.getLocation(),localPlayer, Flags.VILLAGER_TRADE)) {
            ...
            if (!AsureCore.inst().getConfig().getBoolean("Tweaks.VillagerOptimizer.Enabled",true)) return;
        if (AsureCore.inst().getConfig().getBoolean("Tweaks.VillagerManualBreed.Enabled",true)) {
            ItemStack handItem = event.getPlayer().getInventory().getItemInMainHand();
            if (AsureCore.inst().getConfig().contains("Tweaks.VillagerManualBreed.Items."+handItem.getType())) {
                ...
                if (timerVariable.getRight() > AsureUtils.getCurrentTick()) {
                    ...
                    event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"villager_manual_breed_cooldown",Long.toString(remainingTime)));
                    return;
                }

                event.setCancelled(true);
                int amount = handItem.getAmount();
                int requiredAmount = AsureCore.inst().getConfig().getInt("Tweaks.VillagerManualBreed.Items."+handItem.getType());
                if (amount >= requiredAmount) {
                    ...
                    event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),
        ...
        Material interactMaterial = Material.valueOf(AsureCore.inst().getConfig()
                .getString("Tweaks.VillagerOptimizer.InteractMaterial",Material.SHEARS.name()).toUpperCase());
        @Subst("block.chain.hit") String interactSound = AsureCore.inst().getConfig()
        ...
        if (villager.isAware()) {
            if (event.getPlayer().getInventory().getItemInMainHand().getType() == interactMaterial) {
                ...
                if (AsureCore.inst().getConfigManager().getInjectedVillagerTrades().getRecipes().containsKey(professionKey)) {
                    List<MerchantRecipe> recipesToAdd = new ArrayList<>(AsureCore.inst().getConfigManager().getInjectedVillagerTrades().getRecipes().get(professionKey));
                ...
                event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"villager_optimizer_interact_off"));
            ...
            if (event.getPlayer().getInventory().getItemInMainHand().getType() == interactMaterial) {
                ...
                if (interactSound != null)
                    villager.getWorld().playSound(villager.getLocation(),interactSound,SoundCategory.NEUTRAL, 1f,1f);
                event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(),"villager_optimizer_interact_on"));
                ...
                if (!isJobSiteNearby(villager)) {
                    event.getPlayer().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "villager_optimizer_too_far_from_station"));
                    event.setCancelled(true);
                    return;
                }
    ...
    public void onVillagerInventoryClick(InventoryClickEvent event) {
        ...
        if (!AsureCore.inst().getConfig().getBoolean("Tweaks.VillagerOptimizer.Enabled", true)) return;
        if (villager.isAware() && AsureCore.inst().getConfig()
                .getBoolean("Tweaks.VillagerOptimizer.DisableUnoptimizedTrades",true)) {
            String interactMaterialString = AsureCore.inst().getConfig().getString("Tweaks.VillagerOptimizer.InteractMaterial","STONE");
            ...
            event.getWhoClicked().sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "villager_optimizer_interact_hint",translatableKey));
            event.setCancelled(true);
        }
    }
    ...
    private void refreshTrades(Villager villager) {
        ...
        long restockInterval = AsureCore.inst().getConfig().getLong("Tweaks.VillagerOptimizer.RestockInterval",1000L);

        if (AsureUtils.getCurrentTick() < lastRestock + restockInterval) return;

        this.lastRestock.put(villager,AsureUtils.getCurrentTick());
