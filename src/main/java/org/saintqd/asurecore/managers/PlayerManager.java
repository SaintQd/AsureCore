...
import org.saintqd.asurecore.events.HighPingDetectedEvent;
...
public class PlayerManager {
    ...
    public void loadParams(Plugin plugin) {
        ...
        if (plugin.getConfig().getBoolean("PingAlert.Enabled") && pingAlertCheckTask == null) {
            Bukkit.getScheduler().runTaskTimer(plugin,() -> {
                ...
                if (average > plugin.getConfig().getDouble("PingAlert.AlertThreshold")) {
                    plugin.getLogger().warning(AsureLib.inst().getLangManager().getLangLines().get(Key.key(
                            "asurecore:ping_alert_message")).replace("{1}", Integer.toString(average)));
        ...
        if (plugin.getConfig().getBoolean("Tweaks.PhantomsToggle.Enabled") && phantomsToggleTask == null) {
            Bukkit.getScheduler().runTaskTimer(plugin,() -> {
                for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
                    if (onlinePlayer.permissionValue("asurecore.phantomsdisabled") == TriState.TRUE)
        ...
        if (plugin.getConfig().getLong("WorldGuardFlags.Glide.CheckTask",40L) > 0 && glideCheckTask == null && AsureCore.inst().isWorldGuardEnabled()) {
            Bukkit.getScheduler().runTaskTimer(plugin,() -> {
                for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
    ...
    public void applyKnockout(Player player, Player killerPlayer) {
        ...
        knockoutPlayers.put(uuid, AsureUtils.getCurrentTick());
        int period = AsureCore.inst().getConfig().getInt("Tweaks.DeathKnockout.KnockoutPeriod",1200);
        long originalTimeInSeconds = period / 20;
        Location originalLocation = player.getLocation();
        player.showTitle(Title.title(
                AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "death_knockout_title"),
                AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "death_knockout_subtitle",Long.toString(originalTimeInSeconds)),
        ...
        String nicknameFormat = AsureCore.inst().getConfig().getString("Messages.NicknameFormat", player.getName());
        String playerNicknameFormat = (AsureCore.inst().getPlaceholders() != null)
                ? PlaceholderAPI.setPlaceholders(player, PlaceholderAPI.setPlaceholders(player,nicknameFormat))
                : nicknameFormat;
        String killerNicknameFormat = (AsureCore.inst().getPlaceholders() != null)
        ...
        player.sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "death_knockout_message",
                playerNicknameFormat, killerNicknameFormat, Long.toString(originalTimeInSeconds)));
        audience.sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "death_knockout_message",
                playerNicknameFormat, killerNicknameFormat, Long.toString(originalTimeInSeconds)));
        ...
        BukkitRunnable runnable = new BukkitRunnable() {
            ...
            public void run() {
                ...
                long timeUntilRestore = knockoutPlayers.getOrDefault(uuid,AsureUtils.getCurrentTick()) + period - currentTick;
                if (timeUntilRestore > 0 && knockoutPlayers.containsKey(uuid)) {
                    ...
                    if (playerByUuid != null && playerByUuid.isValid()) {
                        ...
                        playerByUuid.showTitle(Title.title(
                                AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "death_knockout_title"),
                                AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "death_knockout_subtitle", Long.toString(timeInSeconds)),
                    ...
                    if (playerByUuid != null && playerByUuid.isValid()) {
                        ...
                        Audience newAudience = Audience.audience(playerByUuid.getNearbyEntities(25,25,25).stream()
                                .filter(entity -> entity instanceof Player).toList());
                        playerByUuid.sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "death_knockout_message_restore",
                                playerNicknameFormat,Long.toString(originalTimeInSeconds)));
                        newAudience.sendMessage(AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "death_knockout_message_restore",
                                playerNicknameFormat,Long.toString(originalTimeInSeconds)));
                    }
                    this.cancel();
                }
