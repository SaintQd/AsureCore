...
public class HintManager {
    ...
    public void loadHints(AsureCore plugin) {
        hints.clear();
    ...
    public void sendHint(Audience audience, int index) {
        ...
        String hintPrefix = AsureLib.inst().getLangManager().getLangLines().getOrDefault(
                Key.key(AsureCore.inst(),"hint_prefix"),"hint_prefix").replace("{1}",Integer.toString(index));
    ...
    public void sendStarterHint(Audience audience, int index) {
        if (AsureCore.inst().isCMIEnabled()) {
            ...
            audience = audience.filterAudience(testedAudience -> {
                if (testedAudience instanceof Player player && player.permissionValue("asurecore.disablehints") != TriState.TRUE) {
    ...
    public void setupStarterHintTask(Plugin plugin) {
        ...
        long timer = AsureCore.inst().getConfig().getLong("StarterHints.Timer",-1);
        if (timer < 0)
            return;
