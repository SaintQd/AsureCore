...
public class ShulkerAlertManager {
    ...
    public static final NamespacedKey SHULKER_UUID_KEY = new NamespacedKey(AsureCore.inst(), "shulker_uuid");
    ...
    public void loadData(Plugin plugin) {
        ...
        shulkerAlertTask = Bukkit.getScheduler().runTaskTimer(plugin,() -> {
            Bukkit.getOnlinePlayers().stream().filter(player -> !player.hasPermission(
                    "asurecore.shulkerdupealerts.bypass")).forEach(player -> {
    ...
    public void showAlert(Player player) {
        List<Player> alertedPlayers = new ArrayList<>(Bukkit.getOnlinePlayers().stream()
                .filter(possiblePlayer -> possiblePlayer.hasPermission("asurecore.shulkerdupealerts.show")).toList());

        String hoverCommand = AsureCore.inst().getConfig().getString("ShulkerDupeDetector.HoverCommand", "tp {1}")
        ...
        Component smallAlertComponent = AsureLib.inst().getLangManager().parseLangString(AsureCore.inst(), "shulker_dupe_alert",
                player.getName(), hoverText);
        for (Player alertedPlayer : alertedPlayers) {
            if (alertedPlayer.permissionValue("asurecore.shulkerdupealerts.disable") != TriState.TRUE) {
                alertedPlayer.sendMessage(smallAlertComponent);
                if (alertedPlayer.permissionValue("asurecore.shulkerdupealerts.disablesound") != TriState.TRUE) {
                    alertedPlayer.playSound(alertSound, alertedPlayer);
                }
            }
        }
        Location playerLoc = player.getLocation();
        for (String command : AsureCore.inst().getConfig().getStringList("ShulkerDupeDetector.CommandsOnDetect")) {
