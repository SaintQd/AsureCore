...
class CalendarEventsManager {
    ...
    val events = hashMapOf<String, AsureCalendarEvent>()

    class AsureCalendarEvent(val name : String, config: ConfigurationSection) {
    ...
    fun loadTimedEvents(plugin: Plugin) {
        ...
        for (eventName in eventYaml.getKeys(false)) {
            try {
                val event = AsureCalendarEvent(eventName, eventYaml.getConfigurationSection(eventName)!!)
                ...
                if (AsureLib.inst().debugLevel >= 1) ex.printStackTrace()
            }
        }
    }
}